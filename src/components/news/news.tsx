...
export default component$(() => {
  ...
  const searchChanged = $(() => {
    fetch(`https://newsapi.org/v2/everything?q='diabetes ${state.searchValue}'&from=2022-11-09&to=2022-11-09&sortBy=popularity&apiKey=${NEWS_API_KEY}`)
    .then(res => res.json())
  ...
  return (
    <div>
      ...
      <input
        ...
        onInput$={(event) => (state.searchValue = (event.target as HTMLInputElement).value)}
      />
      </div>
      ...
      {(state.newsList || []).map((news: NewsModel) => {
        // TODO: Refactor in a new component 
         return(<div>
              <li>[{news.source.name}] {news.title}</li>
         </div>)
