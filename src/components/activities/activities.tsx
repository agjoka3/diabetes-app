import {
 ...
 export const Activities = component$(() => {
  ...
  const submitActivity = $(async () => {
    try {
      const exerciseCalories = state.exercises.find(e => e.id === state.exercise);
      const calories = Number(state?.exerciseLength) / Number(exerciseCalories?.time) * Number(exerciseCalories?.calories);
      console.log('JOO ', state.exercise);
        ...
        onResolved={() => {
          return (
            <form>
             ...
             <label style="font-size: 12px"  for="exercise">Type</label>
              <select style="width: 100px;" name="exercise" id="exercise" value={state.exercise} onChange$={handleInputChange}>
             ...
             <label style="font-size: 12px" for="exerciseLength">Length</label>
              <input style="width: 50px" type="number" id="exerciseLength" value={state.exerciseLength} name="exerciseLength" onChange$={handleInputChange}/>
             </div>
             <div style="float:left; margin-right:20px; width: 50px">
             <label style="font-size: 12px" for="exerciseDate">Date</label>
              <input type="date" id="exerciseDate" value={state.exerciseDate.toString()} name="exerciseDate" onChange$={handleInputChange}/>
             </div>
             <div style="float: rigth, border: 1px solid black; width: 30px">        
                <button onClick$={submitActivity} style={"background: white;  border: none;"}>Submit</button>
